...
module LogStash
  module Outputs
    class RabbitMQ < LogStash::Outputs::Base
      ...
      config :properties, :validate => :hash, :required => false
      ...
      def publish(event, message)
        ...
        @hare_info.exchange.publish(message, :routing_key => event.sprintf(@key), :properties => parse_properties)
      ...
      def parse_properties
        message_props = {:persistent => @persistent}
        if !@properties.nil?
            @properties.each do |key, value|
               message_props[key.to_sym] = value
            end
        end
